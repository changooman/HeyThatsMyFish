...
class GameState:
    ...
    def move_penguin(self, origin, destination, state, player):
        ...
        # print 'destination tile is ', destination_tile.xy
        if self.is_valid_move(state.hexboard.hexagons, origin, destination, state.hexboard, player):
            ...
            destination_tile.penguincolor = player["color"]
            # print player["color"]
            ...
            for cur_player in state.players:
                if cur_player["color"] == player["color"]:
                    start = 0
                    for peng in cur_player["places"]:
                        if peng == origin_tile.xy:
                            cur_player["places"][start] = destination_tile.xy
                        start += 1
    ...
    def is_valid_move(self, hexagons, origin_index, destination_index, hexboard, player):
        ...
        # print 'origin tileeeeeeeeee: ', origin_tile.xy
        x2 = 0
        for x in hexagons:
            if x.spottaken:
                pass
        ...
        if origin_tile.spottaken and origin_tile.xy in player["places"]:
