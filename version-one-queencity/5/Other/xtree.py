...
class HexBoard:
    ...
    def connect_lines2(self, directions_list, start_x, start_y, greatest_possibility):
        valid_hexagons = []
    ...
    def draw_connection2(self, start_x, start_y):
        bot_left = [3, 4]
        bot_right = [1, 2]
        top = [0, 5]
        bot = [2, 3]
...
def main():
    ...
    try:
        contents = input()
        total = 0
        players = contents["state"]["players"]
        if players is []:
            print 'false'
        else:
            hexboard2 = contents["state"]["board"]
            maxrowlen = 0
            for x in hexboard2:
                if len(x) > maxrowlen:
                    maxrowlen = len(x)
            ...
            tempstate = create_state(players, hexboard)
            index = 1
            origin = None
            destination = None
                ...
                if a_player["places"] != []:
                    for penguin in a_player["places"]:
                        ...
                        for hex in hexboard.hexagons:
                            ...
                            start += 1
            for hex in hexboard.hexagons:
                if hex.xy == contents["from"]:
                    origin = index
                if hex.xy == contents["to"]:
                    destination = index
                index += 1
            # print 'origin is: ', origin
            # print 'destination is: ', destination
            tempstate.move_penguin(origin, destination, tempstate, contents["state"]["players"][0])
            index2 = 1
            origin2 = 0
            for hex in hexboard.hexagons:
                if hex.xy == hexboard.hexagons[destination - 1].xy:
                    origin2 = index2
                index2 += 1
            origin_tile = hexboard.hexagons[int(origin2) - 1]
            # print 'origin tile is: ', origin_tile.xy
            neighbors = hexboard.draw_connection(origin_tile.x, origin_tile.y)
            # for neigh in neighbors:
            #     print 'new origin tile neighbors are: ', neigh.xy
            validpenguins = []
            for penguin2 in contents["state"]["players"][1]["places"]:
                origin3 = 0
                index2 = 1
                for hex in hexboard.hexagons:
                    if hex.xy == penguin2:
                    ...
                    index2 += 1
                if origin3 != -1:
                    origin_tile2 = hexboard.hexagons[int(origin3) - 1]
                    # print 'the 3rd penguin is: ', origin_tile2.xy
                    neighbors2 = hexboard.draw_connection2(origin_tile2.x, origin_tile2.y)
                    intersection = set(neighbors).intersection(neighbors2)
                    if len(intersection) >= 1:
                        validpenguins.append(penguin2)
            if len(validpenguins) == 0:
                print 'false'
            elif len(validpenguins) == 1:
                actualrelease = [validpenguins[0], neighbors[0].xy]
                print actualrelease
            else:
                topmostrowfrom = contents["from"][0]
                leftmostcolumnfrom = contents["from"][1]
                topmostrowto = contents["to"][0]
                topmostcolumnto = contents["to"][1]
                order = [topmostrowfrom, leftmostcolumnfrom, topmostrowto, topmostcolumnto]
                found = False
                indextrack = 0
                while not found:
                    newpeng = []
                    for pengy in validpenguins:
                        indextrack = 0
                        for hex in hexboard.hexagons:
                            if hex.xy == pengy:
                                newpeng.append((indextrack, hex.xy))
                            indextrack += 1
                    found = True
                newpeng.reverse()
                # print 'NEW PENG IS: ', newpeng
                release = newpeng[0][1]
                actualrelease = [release, neighbors[0].xy]
                print actualrelease
    except:
        print 'false'
main()
