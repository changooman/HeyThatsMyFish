...
class Player:
    """
        ...
        * Examples: 3, 45, 23
        * @param penguins A list of penguins, represented by coordinates, that are owned by a player
        * Examples: (1,1) (2,3) (0,0)
        */
    """
    ...
    def get_score(self, player, state):
        for a_player in state.players:
            # print a_player.color
            if a_player.color == player.color:
                # print "found it!"
                return a_player.score
...
class GameState:
    ...
    def __init__(self, players, board):
        ...
        self.player_index = 0   # index for the player who's turn it is during this state
    ...
    def place_penguin(self, hexboard, player, penguinsleft):
        penguinsleft = penguinsleft
        hex_index = self.penguin_index(hexboard)
        while penguinsleft != 0:
            hexagon = hexboard.hexagons[hex_index]
            ...
            hex_index += len(self.players)
            penguinsleft += -1


    # gets the starting hexagon index that a player's first penguin will be placed on
    # @param hexboard, the given hexboard that the index will be taken from
    # @return index, the index of the hexagon that the first penguin will be placed on
    def penguin_index(self, hexboard):
        index = 0
        for hexagon in hexboard.hexagons:
            if hexagon.spottaken:
                index += 1
            else:
                return index
    ...
    def move_penguin(self, origin, destination, state, player):
        ...
        if self.is_valid_move(state.hexboard.hexagons, origin, destination, state.hexboard, player):
            ...
            destination_tile.penguincolor = state.players[player].color
            # self.move_render(origin, origin_tile, destination, destination_tile, state, player)
            state.players[player].score += destination_tile.fish
            # print 'score is: ', state.players[player].score
            for cur_player in state.players:
                if cur_player.color == self.players[player].color:
                    start = 0
    ...
    def is_valid_move(self, hexagons, origin_index, destination_index, hexboard, player):
        ...
        if origin_tile.spottaken and origin_tile.xy in self.players[player].penguins:
            if not destination_tile.sunk and not destination_tile.spottaken:
                if destination_tile in valid_destinations:
                    # print 'destination xy is valid: ', destination_tile.xy
                    return True
                else:
                    return False
            else:
                # print "destination not valid: ", destination_tile.xy, ' ', destination_tile.sunk, ' ', destination_tile.spottaken
                # print "Destinaiton is not free"
                return False
        else:
            return False
...
if __name__ == "__main__":
    ...
    hex_board = board.HexBoard(30, 4, 3, 4 * 3, [(1, 1), (2, 2)])
    print "aaahhh", hex_board
    tempstate = create_state(players, hex_board)
    print hex_board
    score = tempstate.get_score(player1, tempstate)
    print score
