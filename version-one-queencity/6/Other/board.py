...
class HexBoard:
    ...
    def __init__(self, size, row, col, total, holes):
    ...
    def find_neighbors(self, current):
        ...
        neighbors = []
        for hexagon in self.hexagons:
            print 'CURRENTCOORDS: ', current_coords
            print 'current xy: ', current.xy
            print 'HEX xy: ', hexagon.xy
            print 'AGAINST: ', hexagon.coordinates
        ...
        print 'neighbors for: ', current.xy, ' ', len(neighbors)
        return neighbors
...
class RenderHexBoard(Tk):
    ...
    def render(self, hexboard):
        ...
        for hex in hexboard.hexagons:
            try:
                filename = '{}.gif'.format(hex.penguincolor)
            except:
                filename = 'black.gif'
            photo = PhotoImage(file=filename)
            self.photocache.append(photo)
...
if __name__ == '__main__':
    hex_board = HexBoard(30, 7, 3, 7 * 3, [])
    statetemp = state.create_state([], hex_board)
    for h in hex_board.hexagons:
        print h.xy
        if h.xy == [0, 1]:
            h.sunk = True
    render = RenderHexBoard(statetemp.hexboard)
    render.render_state()
    # thing = hex_board.find_neighbors(hex_board.hexagons[3])