...
class HexBoard:
    ...
    def __init__(self, size, row, col, total, allvalues):
        ...
        self.allvalues = allvalues  # Holes that should be on the board.
    ...
    def set_xboardtiles(self, limit, total):
        holes = []
        last = 0
        ...
        for hex in self.hexagons:
            if self.allvalues[startup] == 0:
                hex.sunk = True
            else:
                hex.fish = self.allvalues[startup]
            startup += 1
        hexagons2 = []
        if self.rows % 2 == 0:
            start = 0
            for i in range(self.rows):
                if i % 2 == 0:
                    for x in range(self.cols):
                        hexagons2.append(hex_board.hexagons[start])
                        hexagons2.append(hex_board.hexagons[start + self.cols])
                        start += 1
                    start += hex_board.cols
        else:
            start = 0
            for i in range(self.rows):
                if i % 2 == 0 and i != self.rows - 1:
                    for x in range(self.cols):
                        hexagons2.append(self.hexagons[start])
                        hexagons2.append(self.hexagons[start + self.cols])
                        start += 1
                    start += self.cols
                if i == self.rows - 1:
                    left = self.cols
    ...
    def init_gridxboard(self, total):
        ...
        self.set_xboardtiles(limit, total)
    ...
    def fill_tile(self, c, r, size, holes, offset):
        ...
        if r % 2 == 0:
            ...
            offset = self.size / 4 - size + 4

        h = FillHexagon(c * (size * 3) - offset + size * 2,
                        (r * (size - 4)) + (size / 2),
                        size,
                        [r, c], is_sunk)
    ...
    def find_neighbors(self, current):
        ...
        for hexagon in self.hexagons:
            ...
            if len(intersection) == 2 and not hexagon.sunk and not hexagon.spottaken:
...
def main():
    ...
    try:
        ...
        total = 0
        layers = contents[0]
        checkplayers = contents[1]["players"]
        if checkplayers is []:
            print 'false'
        else:
            hexboard2 = contents[1]["board"]
            maxrowlen = 0
            for x in hexboard2:
                if len(x) > maxrowlen:
                    maxrowlen = len(x)
            ...
            for x in hexboard2:
                if type(x) == list:
                    for num in x:
                        total += 1
            allvalues = []
            for tilevalue in contents[1]["board"]:
                for ttv in tilevalue:
                    allvalues.append(ttv)
            rowcount = 0
            for row in contents[1]["board"]:
                rowcount += 1
            colcount = 0
            for row in contents[1]["board"]:
                if len(row) > colcount:
                    colcount = len(row)
            hexboard = HexBoard(30, rowcount, colcount, total, allvalues)
            agestart = 1
            player_list = []
            for a_player in checkplayers:
                player = Player(a_player["color"], agestart)
                player.penguins = a_player["places"]
                player_list.append(player)
                agestart += 1
            for a_player in checkplayers:
                if a_player["places"] != []:
                    for penguin in a_player["places"]:
                        start = 0
                        for hex in hexboard.hexagons:
                            if hex.xy == penguin and not hex.sunk:
                                hexboard.hexagons[start].spottaken = True
                                hexboard.hexagons[start].penguincolor = a_player["color"]
                            start += 1
            statetemp = state.create_state(player_list, hexboard)
            interface = PlayerInterface(statetemp, player_list[0], player_list)
            # render = RenderHexBoard(statetemp.hexboard)
            # render.render_state()
            game = Game(statetemp.hexboard, player_list, 0, layers)
            game_tree = game.game_tree(statetemp)
            try:
                bestmove = interface.best_move(game_tree, player_list[0], layers)
                bestmove2 = [statetemp.hexboard.hexagons[bestmove[0] -1].xy, statetemp.hexboard.hexagons[bestmove[1] -1].xy]
                print bestmove2
            except:
                print 'false'
    except:
