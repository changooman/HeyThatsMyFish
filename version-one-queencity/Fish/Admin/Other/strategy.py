...
class PlayerInterface:
    ...
    def best_path(self, game_tree, player, last_layer_index):
        all_paths, origin_list = self.get_all_paths(game_tree)
        best_path = self.check_best_path(origin_list, player, game_tree)
        scorelist = self.check_scores(best_path, player, last_layer_index)
    ...
    # Example: [1,2,4,-1,4,0]
    def check_scores(self, best_path, player, last_layer_index):
        scorelist = []
        index = 0
        for node in best_path:
            if node[0] == last_layer_index:
                state_score = 0
                ...
                index += 1
        if scorelist == [] and last_layer_index >= 1:
            scorelist = self.check_scores(best_path, player, last_layer_index - 1)
        elif scorelist == [] and last_layer_index < 1:
    ...
    #  ("red", 12)
    # @return best_move
    def best_move(self, game_tree, player, layers):
        best_path = self.best_path(game_tree, player, layers)
        for node in best_path:
            if node[0] == 1:
                return node[3]
        return
...
if __name__ == "__main__":
    ...
    print("thing: ", interface.best_move(game_tree, player1, layers))
