...
class Game:
    ...
    # Purpose: Gets the first layer of a tree of states from a given state by checking the possible moves for all
    # penguins owned by the current player, checking the possible moves, and creating a state instance for each
    # possible move.
    ...
    def get_next_state(self, tree_node, action):
        ...
        if action[0] == 'Move':
            if tree_node.is_valid_move(self.hexagons, action[1], action[2], next_state.hexboard, self.player):
                next_state.move_penguin(action[1], action[2], next_state, self.player)
                return next_state
            else:
...
if __name__ == "__main__":
    ...
    hex_board = xboard.HexBoard(30, 4, 3, 4 * 3, [])
