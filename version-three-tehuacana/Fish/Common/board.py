...
class Board():
    ...
    def __init__(self, rows, cols, holes=[], min_one_fish=0, uniform=False, uniform_num_fish=0):
        ...
        holes:  A list of 2D coordinate tuples where holes should appear on the game board, 0 indexed. If a given hole
            is not a valid position in the board, then the hole is ignored.
        min_one_fish:   The minimum number of tiles that should have one fish on them. Placed starting from the top left (0, 0), and
            placed across rows.
        uniform:    Whether or not to place the same number of fish on every tile and have no holes. If this option is true, `holes`
             and `min_one_fish` is ignored.
        uniform_num_fish:   The number of fish to place on each tile if the board is uniform. Ignored if `uniform` is False.
        """
        if rows < 1:
    ...
    def get_holes_in_board(self):
        """Get the number of holes in this board

        return: (int) the number of holes in this board
        """

        holes = 0
        for i in range(self.get_rows()):
            for j in range(self.get_cols()):
                if self.get_tile((i, j)) is None:
                    holes += 1
